...
//		System.out.println(new File("E:\\yousayidoprop.rar").toPath());
		...
		System.out.println(Files.probeContentType(new File("C:\\Users\\Crap\\eclipse-workspace\\wxbot\\client\\src\\main\\resources\\view\\lib\\iview-3\\styles\\fonts\\ionicons.woff").toPath()));
		String path = "jar:file:/C:/Users/Crap/eclipse-workspace/wxbot/client/target/client-0.0.1-SNAPSHOT.jar!/view/lib/iview-3/styles/fonts/ionicons.ttf?v=3.0.0";
		path = path.split("[?]")[0];
		System.out.println(path.substring(path.lastIndexOf(".")+1));
		
		Set<Contact> set = Stream.of("123","abc").map(s -> {
			Contact c = new Contact();
			c.setUserName(s);
			return c;
		}).collect(Collectors.toSet());
		
		Contact x = set.stream().filter(c -> c.getUserName().equals("123")).findFirst().orElse(null);
		x.setUserName("456");
		set.forEach(e -> {
